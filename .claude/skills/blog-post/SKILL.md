---
name: blog-post
description: End-to-end pipeline for writing and publishing a post on lizard-spock.co.uk. Covers narrative structure, the technical-writer passes, image generation in the site aesthetic, build verification, and deploy. Use when writing a new post, converting a draft into a post, or adding images to an existing post.
---
...
Invoke `technical-writer` and follow its four passes: structure, draft, refine,
review. Do not restate its style guide here.
...
5. **Read every generated image back and check it against the spec before accepting it.** This step is not optional. Image models produce technically wrong diagrams that look plausible: contradictory arrow directions, wrong element counts, invented labels. The model's own self-report of its errors is unreliable, so verify by looking.
6. Regenerate failures with tighter constraints. If a second attempt fails, build it as a `dot` diagram instead.
7. Produce the display pair:
...
8. Embed as a linked pair so the full resolution opens on click:
